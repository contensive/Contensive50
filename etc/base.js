...
function cjAjaxURL(LocalURL,FormID,DestinationID,onEmptyHideID,onEmptyShowID) {
	...
	if(pos!=-1){url=url.substr(0,pos)}
	pos = url.indexOf("://");
	if(pos!=-1){pos=url.indexOf("/",pos+3);
	if(pos!=-1){url=url.substr(0,pos)}}
	url+=LocalURL;
	pos=url.indexOf("?")
...
//					***** if provided, the call will POST to the url, url should be index.php, etc.
...
function cjRemote( options ) {
	var url,fo,ptr,e,ret,pos,isPost;
	isPost=false;
	if(options.url) {
		url=options.url;
	} else {
		if (options.formId) {
			// POST must be to actual page, not 404 remote name
			url="/";
		} else {
			url=document.URL;
		}
	...
	url+='?requestbinary=true&nocache='+Math.random()
	if (options.method) { url+='&remotemethodaddon='+options.method }
	if (options.queryString) { url+='&'+options.queryString }
	if (options.formId) {
		// post forms, otherwise get
		// url should be set to host page (index.php)
		var data = new FormData(document.getElementById(options.formId));
		//var data = new FormData(jQuery('#'+options.formId));
		var opts = {
			url: url,
			data: data,
			cache: false,
			...
			success: function( serverResponse ){ 
				cjRemoteSuccess ( serverResponse, options )
			}
		}
		if (data.fake) {
			// use FormData extension included at bottom of core.js for non-Html5 browsers
...
function cjRemoteSuccess( serverResponse, options ) {
...
function cjRemoteLegacy( options ) 
{
 ...
 * from https://github.com/francois2metz/html5-formdata
 */
(function(w) {
    if (w.FormData)
        return;
