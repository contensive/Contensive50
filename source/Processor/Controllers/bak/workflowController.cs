...
namespace Controllers {
    ...
    public class workflowController : IDisposable {
        ...
        private DateTime main_EditLockDateExpires_Local;
        ...
        // '' <remarks></remarks>
        public workflowController(coreClass cpCore) {
            try {
                // 
                this.cpCore = cpCore;
                constructed = true;
        ...
        public bool GetEditLockStatus(string ContentName, int RecordID) {
            try {
                ...
                main_EditLockStatus_Local = this.getEditLock(genericController.encodeText(ContentName), genericController.EncodeInteger(RecordID), ReturnMemberID, ReturnDateExpires);
                if ((main_EditLockStatus_Local 
                            && (ReturnMemberID != cpCore.doc.authContext.user.id))) {
        ...
        public string GetEditLockMemberName(string ContentName, int RecordID) {
            try {
                int CS;
                // 
                if ((main_EditLockContentRecordKey_Local 
                            != (ContentName + genericController.encodeText(RecordID)))) {
                    this.GetEditLockStatus(ContentName, RecordID);
                }
                
                if (main_EditLockStatus_Local) {
                    if ((main_EditLockMemberName_Local == "")) {
        ...
        public void SetEditLock(string ContentName, int RecordID) {
            this.setEditLock(genericController.encodeText(ContentName), genericController.EncodeInteger(RecordID), cpCore.doc.authContext.user.id);
        }
        ...
        public void ClearEditLock(string ContentName, int RecordID) {
            this.clearEditLock(genericController.encodeText(ContentName), genericController.EncodeInteger(RecordID), cpCore.doc.authContext.user.id);
        }
        ...
        public bool isRecordLocked(string ContentName, int RecordID, int MemberID) {
            ...
            try {
                ...
                int CS;
                // 
                Criteria = (this.getAuthoringControlCriteria(ContentName, RecordID) + ("and(CreatedBy<>" 
                            + (cpCore.db.encodeSQLNumber(MemberID) + ")")));
        ...
        private string getAuthoringControlCriteria(string ContentName, int RecordID) {
            ...
            try {
                ...
                // 
                ContentCnt = 0;
                CS = cpCore.db.csOpen("Content", ("(contenttableid=" 
                                + (TableID + ")")));
                ...
                if ((ContentCnt < 1)) {
                    ...
                    // 
                    throw new ApplicationException(("TableID [" 
                                    + (TableID + "] could not be found in any ccContent.ContentTableID")));
                    result = "(1=0)";
                }
                    ...
                    // 
                    result = ("(ContentID=" 
                                + (cpCore.db.encodeSQLNumber(EncodeInteger(Criteria.Substring(1))) + (")And(RecordID=" 
                                + (cpCore.db.encodeSQLNumber(RecordID) + (")And((DateExpires>" 
                                + (cpCore.db.encodeSQLDate(Now) + ")Or(DateExpires Is null))"))))));
                }
                else {
                    // 
                    ...
                    // 
                    result = ("(ContentID In (" 
                                + (Criteria.Substring(1) + ("))And(RecordID=" 
                                + (cpCore.db.encodeSQLNumber(RecordID) + (")And((DateExpires>" 
                                + (cpCore.db.encodeSQLDate(Now) + ")Or(DateExpires Is null))"))))));
                }
        ...
        public void clearAuthoringControl(string ContentName, int RecordID, int AuthoringControl, int MemberID) {
            try {
                string Criteria;
                // 
                Criteria = (this.getAuthoringControlCriteria(ContentName, RecordID) + ("And(ControlType=" 
                            + (AuthoringControl + ")")));
                switch (AuthoringControl) {
                    case AuthoringControlsEditing:
                        cpCore.db.deleteContentRecords("Authoring Controls", (Criteria + ("And(CreatedBy=" 
                                        + (cpCore.db.encodeSQLNumber(MemberID) + ")"))), MemberID);
                        break;
        ...
        public void setRecordLocking(string ContentName, int RecordID, int AuthoringControl, int MemberID) {
            try {
                ...
                string AuthoringCriteria;
                int CSNewLock;
                int CSCurrentLock;
                string sqlCriteria;
                double EditLockTimeoutDays;
                double EditLockTimeoutMinutes;
                ...
                if ((ContentID != 0)) {
                    ...
                    AuthoringCriteria = this.getAuthoringControlCriteria(ContentName, RecordID);
                    switch (AuthoringControl) {
                        case AuthoringControlsEditing:
                            EditLockTimeoutMinutes = EncodeNumber(cpCore.siteProperties.getText("EditLockTimeout", "5"));
                            if ((EditLockTimeoutMinutes != 0)) {
                                sqlCriteria = (AuthoringCriteria + ("And(ControlType=" 
                                            + (AuthoringControlsEditing + ")")));
                                EditLockTimeoutDays = (EditLockTimeoutMinutes / (60 / 24));
                                cpCore.db.deleteContentRecords("Authoring Controls", (sqlCriteria + ("And(DATEEXPIRES<" 
                                                + (cpCore.db.encodeSQLDate(Now) + ")"))));
                                // 
                                ...
                                if (!cpCore.db.csOk(CSCurrentLock)) {
                                    ...
                                    if (cpCore.db.csOk(CSNewLock)) {
                                        ...
                                        cpCore.db.csSet(CSNewLock, "DateExpires", Now.AddDays(EditLockTimeoutDays));
                                ...
                                else if ((cpCore.db.csGetInteger(CSCurrentLock, "CreatedBy") == MemberID)) {
                                    // 
                                    //  Record Locked by Member, update DateExpire
                                    // 
                                    cpCore.db.csSet(CSCurrentLock, "DateExpires", Now.AddDays(EditLockTimeoutDays));
                                }
        ...
        public void setEditLock(string ContentName, int RecordID, int MemberID) {
            try {
                this.setEditLock2(ContentName, RecordID, MemberID, false);
            }
            catch (Exception ex) {
                cpCore.handleException(ex);
                throw;
        ...
        public void clearEditLock(string ContentName, int RecordID, int MemberID) {
            try {
                this.setEditLock2(ContentName, RecordID, MemberID, true);
            }
            catch (Exception ex) {
                cpCore.handleException(ex);
                throw;
        ...
        public bool getEditLock(string ContentName, int RecordID, ref int ReturnMemberID, ref DateTime ReturnDateExpires) {
            return this.getEditLock2(ContentName, RecordID, ReturnMemberID, ReturnDateExpires);
        }
        ...
        // =================================================================================
        // 
        public void setEditLock2(string ContentName, int RecordID, int MemberID, bool ClearLock, void =, void False) {
            try {
                int SourcePointer;
                // Warning!!! Optional parameters not supported
                double EditLockTimeoutMinutes;
                bool LockFound;
                DateTime EditLockDateExpires;
                string SourceKey;
                DateTime SourceDateExpires;
                ...
                if (((ContentName != "") 
                    ...
                    if ((EditLockCount > 0)) {
                        for (SourcePointer = 0; (SourcePointer 
                                    <= (EditLockCount - 1)); SourcePointer++) {
                            if (false) {
                                break;
                            }
                    ...
                    if ((!LockFound 
                        ...
                        // 
                        if ((EditLockCount > 0)) {
                            if ((DestinationPointer > UBound(EditLockArray))) {
                                object Preserve;
                                EditLockArray[(DestinationPointer + 10)];
                            }
