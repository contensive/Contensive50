...
namespace Controllers {
    ...
    public class cacheController : IDisposable {
        ...
        private DateTime globalInvalidationDate {
            get {
                ...
                if ((_globalInvalidationDate == null)) {
                    cacheWrapperClass dataObject = this.getWrappedContent("globalInvalidationDate");
                    if (dataObject) {
                        IsNot;
                        null;
                    ...
                    if ((_globalInvalidationDate == null)) {
                        ...
                        setWrappedContent("globalInvalidationDate", new cacheWrapperClass(), With, {, ., saveDate=CDate(_globalInvalidationDateUnknown);
                    }
                ...
                else if ((DateTime.Parse(_globalInvalidationDate).CompareTo(new DateTime(1990, 8, 7)) < 0)) {
                    _globalInvalidationDate = new DateTime(1990, 8, 7);
                }
                
                return DateTime.Parse(_globalInvalidationDate);
            }
        }
        ...
        public cacheController(coreClass cpCore) {
            try {
                ...
                if (cpCore.serverConfig.enableRemoteCache) {
                    ...
                    if (!string.IsNullOrEmpty(cacheEndpoint)) {
                        string[] cacheEndpointSplit = ":".Split(c);
            ...
            // '' <remarks></remarks>
            setWrappedContent(((string)(key)), ((cacheWrapperClass)(wrappedContent)));
            try {
                if (string.IsNullOrEmpty(key)) {
                    ...
                    if (cpCore.serverConfig.enableRemoteCache) {
                        ...
                        if (cpCore.serverConfig.enableLocalMemoryCache) {
                            ...
                            this.setWrappedContent_MemoryCache(wrapperKey, wrappedContent);
                        }
                        ...
                        if (cpCore.serverConfig.enableLocalFileCache) {
                            ...
                            Using;
                            ((void)(mutex));
                            new System.Threading.Mutex(false, wrapperKey);
                            mutex.WaitOne();
        ...
        public objectClass getObject<objectClass>(string key) {
            ...
            try {
                cacheWrapperClass wrappedContent;
                ...
                if (!string.IsNullOrEmpty(key)) {
                    // 
                    //  -- read cacheWrapper
                    appendCacheLog(("getObject(" 
                                    + (key + "), enter")));
                    Stopwatch sw = Stopwatch.StartNew();
                    wrappedContent = this.getWrappedContent(key);
                    if (wrappedContent) {
                        IsNot;
                        null;
                        // 
                        ...
                        if ((dateCompare >= 0)) {
                            ...
                            //  -- global invalidation
                            appendCacheLog(("GetObject(" 
                                            + (key + ("), invalidated because the cacheObject\'s saveDate [" 
                                            + (wrappedContent.saveDate.ToString() + ("] is after the globalInvalidationDate [" 
                                            + (globalInvalidationDate + "]")))))));
                        }
                        else {
                            // 
                            ...
                            if ((wrappedContent.dependentKeyList.Count == 0)) {
                                ...
                                appendCacheLog(("GetObject(" 
                                                + (key + "), no dependentKeyList")));
                            }
                            else {
                                foreach (string dependentKey in wrappedContent.dependentKeyList) {
                                    cacheWrapperClass dependantObject = this.getWrappedContent(dependentKey);
                                    if (dependantObject) {
                                        IsNot;
                                        null;
                                        ...
                                        if ((dateCompare >= 0)) {
                                            cacheMiss = true;
                                            appendCacheLog(("GetObject(" 
                                                            + (key + ("), invalidated because the dependantobject [" 
                                                            + (dependentKey + ("] has a saveDate [" 
                                                            + (dependantObject.saveDate.ToString() + ("] after the cacheObject\'s dateDate [" 
                                                            + (wrappedContent.saveDate.ToString() + "]")))))))));
                                            break;
                                        }
                            ...
                            if (!cacheMiss) {
                                if (!string.IsNullOrEmpty(wrappedContent.key)) {
                                    ...
                                    result = this.getObject(Of, objectClass)[wrappedContent.key];
                                }
                                else if ((wrappedContent.content.GetType() == Newtonsoft.Json.Linq.JObject)) {
                                    // 
                                    //  -- newtonsoft types
                                ...
                                else if ((wrappedContent.content.GetType() == Newtonsoft.Json.Linq.JArray)) {
                                    // 
                                    //  -- newtonsoft types
                                    Newtonsoft.Json.Linq.JArray data;
                                    wrappedContent.content;
                                    Newtonsoft.Json.Linq.JArray;
                                    result = data.ToObject(Of, objectClass)[];
                                }
                                else if (((wrappedContent.content.GetType() == String) 
                                            && result.GetType())) {
                                    IsNot;
                                    String;
                                    // 
        ...
        private cacheWrapperClass getWrappedContent(string key) {
            ...
            try {
                appendCacheLog(("getWrappedContent(" 
                                + (key + "), enter ")));
                if (string.IsNullOrEmpty(key)) {
                    ...
                    if (remoteCacheInitialized) {
                        ...
                        if (result) {
                            IsNot;
                            null;
                            appendCacheLog(("getCacheWrapper(" 
                                            + (key + "), remoteCache hit")));
                        }
                        else {
                            appendCacheLog(("getCacheWrapper(" 
                                            + (key + "), remoteCache miss")));
                        }
                        
                    }
                    
                    if (((result == null) 
                                && cpCore.serverConfig.enableLocalMemoryCache)) {
                        // 
                        //  -- local memory cache
                        // Dim cache As ObjectCache = MemoryCache.Default
                        MemoryCache.Default(wrapperKey);
                        cacheWrapperClass;
                        if (result) {
                            IsNot;
                            null;
                            ...
                            appendCacheLog(("getCacheWrapper(" 
                                            + (key + "), memoryCache miss")));
                        }
                        
                    }
                    
                    if (((result == null) 
                                && cpCore.serverConfig.enableLocalFileCache)) {
                        // 
                        //  -- local file cache
...
Endclass  {
    ...
    void setContent(string key, object content, DateTime invalidationDate, string dependantKey) {
        try {
    ...
    void setContent(string key, object content, List<string> dependentKeyList) {
        try {
            cacheWrapperClass cacheWrapper = new cacheWrapperClass();
            // With...
            content = content;
    ...
    void setContent(string key, object content, string dependantKey) {
        try {
    ...
    void invalidateAll() {
        try {
            setWrappedContent("globalInvalidationDate", new cacheWrapperClass(), With, {, ., saveDate=Now(Unknown);
            _globalInvalidationDate = null;
        }
        catch (Exception ex) {
            cpCore.handleException(ex);
    ...
    public void invalidateContent(string key) {
        try {
            if (!string.IsNullOrEmpty(key.Trim())) {
                setWrappedContent(key, new cacheWrapperClass(), With, {, ., saveDate=Now(Unknown);
            }
    ...
    private string getWrapperKey(string appName, string key) {
        string result;
        result = (appName + ("-" + key));
        result = result.Replace("0x[a-fA-F\\d]{2}", "_");
    ...
    public string getString(string key) {
        return genericController.encodeText(this.getObject(Of, String)[key]);
    }
    ...
    public static string getCacheKey_Entity(string tableName, string fieldName, string fieldValue) {
        return ((tableName + ("/" 
                    + (fieldName + ("/" + fieldValue))))).ToLower().Replace(" ", "_");
    }
    ...
    public static string getCacheKey_Entity(string tableName, string field1Name, int field1Value, string field2Name, int field2Value) {
        return ((tableName + ("/" 
                    + (field1Name + ("/" 
                    + (field1Value + ("/" 
                    + (field2Name + ("/" + field2Value))))))))).ToLower().Replace(" ", "_");
    }
    ...
    public static string getCacheKey_ComplexObject(string objectName, string uniqueObjectIdentifier) {
        return (("complexobject/" 
                    + (objectName + ("/" + uniqueObjectIdentifier)))).ToLower().Replace(" ", "_");
    }
