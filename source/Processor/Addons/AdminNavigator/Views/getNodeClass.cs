				...
				if (EmptyNodeList.Contains(TopParentNode)) {
					...
					if (cs11.OpenSQL(SQL)) {
						do {
							...
							//Index.SetPointer(SortNodes[SortPtr].Name, SortPtr);
							SortPtr = SortPtr + 1;
							cs11.GoNext();
						}
						while (cs11.OK());
                    ...
                    if (SortNodes.Count == 0) {
						...
						//	if (SortNodes[SortPtr].Name.ToLower() != LastName) {
						//		returnNav = returnNav + GetNode(cp, env, SortNodes[SortPtr].CollectionID, SortNodes[SortPtr].ContentControlID, SortNodes[SortPtr].helpCollectionID, SortNodes[SortPtr].HelpAddonID, SortNodes[SortPtr].ContentID, SortNodes[SortPtr].Link, SortNodes[SortPtr].addonid, 0, SortNodes[SortPtr].Name, LegacyMenuControlID, EmptyNodeList, SortNodes[SortPtr].NavigatorID, SortNodes[SortPtr].NavIconType, cp.Utils.EncodeHTML(SortNodes[SortPtr].NavIconTitle), AutoManageAddons, NodeType, SortNodes[SortPtr].NewWindow, BlockSubNodes, OpenNodeList, SortNodes[SortPtr].NodeIDString, ref NodeDraggableJS, new List<string>());
						//		Return_DraggableJS = Return_DraggableJS + NodeDraggableJS;
						//		LastName = SortNodes[SortPtr].Name.ToLower();
						//	}
