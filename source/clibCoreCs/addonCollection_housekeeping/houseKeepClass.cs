...
namespace Contensive.Core
{
	...
	public class houseKeepClass
	{
		...
		public void HouseKeep(bool DebugMode)
		{
			...
			if (!string.IsNullOrEmpty(Config))
			{
				ConfigLines = Microsoft.VisualBasic.Strings.Split(Config, Environment.NewLine, -1, Microsoft.VisualBasic.CompareMethod.Binary);
...
//INSTANT C# TODO TASK: Calls to the VB 'Err' function are not converted by Instant C#:
				...
				if (false)
				{
					...
					//
					AppendClassLog(cp.core, appName, "HouseKeep_App_Daily(" + appName + ")", "Deleting Calendar Event Rules with bad CalendarID.");
					switch (DataSourceType)
					{
						case DataSourceTypeODBCAccess:
							SQL = "delete ccCalendarEventRules.*"
								+ " From ccCalendarEventRules"
								+ " LEFT JOIN ccCalendars on ccCalendars.ID=ccCalendarEventRules.CalendarID"
								+ " WHERE (ccCalendars.ID is null) ";
							cp.core.db.executeQuery(SQL);
							break;
						case DataSourceTypeODBCSQLServer:
							SQL = "delete from ccCalendarEventRules"
								+ " From ccCalendarEventRules"
								+ " LEFT JOIN ccCalendars on ccCalendars.ID=ccCalendarEventRules.CalendarID"
								+ " WHERE (ccCalendars.ID is null) ";
							cp.core.db.executeQuery(SQL);
					...
					//
					AppendClassLog(cp.core, appName, "HouseKeep_App_Daily(" + appName + ")", "Deleting Calendar Event Rules with bad CalendarEventID.");
					switch (DataSourceType)
					{
						case DataSourceTypeODBCAccess:
							SQL = "delete ccCalendarEventRules.*"
								+ " From ccCalendarEventRules"
								+ " LEFT JOIN ccCalendarEvents on ccCalendarEvents.ID=ccCalendarEventRules.CalendarEventID"
