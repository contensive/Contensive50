	...
	public class authContextModel
	{
					...
					else
					{
						...
						if (!string.IsNullOrEmpty(MemberLinkinEID))
						{
							...
							if (cpCore.siteProperties.getBoolean("AllowLinkLogin", true))
							{
								//
								// -- allow Link Login
								cpCore.security.decodeToken(MemberLinkinEID, MemberLinkLoginID, tokenDate);
							}
							else if (cpCore.siteProperties.getBoolean("AllowLinkRecognize", true))
							{
								//
								// -- allow Link Recognize
								cpCore.security.decodeToken(MemberLinkinEID, MemberLinkRecognizeID, tokenDate);
							}
							else
							{
								//
						...
						if ((visitInit_allowVisitTracking) || (!string.IsNullOrEmpty(visitCookie)) | (MemberLinkLoginID != 0) | (MemberLinkRecognizeID != 0))
						{
							...
							if (!string.IsNullOrEmpty(visitCookie))
							{
								//
								// -- visit cookie found
								cpCore.security.decodeToken(visitCookie, cookieVisitId, visitCookieTimestamp);
								if (cookieVisitId == 0)
								{
									//
							...
							if (cookieVisitId != 0)
							{
								...
								resultAuthContext.visit = Models.Entity.visitModel.create(cpCore, cookieVisitId, new List<string>());
								if (resultAuthContext.visit == null)
								{
									//
									// -- visit record is missing, create a new visit
									resultAuthContext.visit = Models.Entity.visitModel.add(cpCore, new List<string>());
								}
								...
								else
								{
									...
									if (resultAuthContext.visit.VisitorID > 0)
									{
										...
										Models.Entity.visitorModel testVisitor = Models.Entity.visitorModel.create(cpCore, resultAuthContext.visit.VisitorID, new List<string>());
										if (testVisitor != null)
										{
									...
									if (resultAuthContext.visit.MemberID > 0)
									{
										...
										Models.Entity.personModel testUser = Models.Entity.personModel.create(cpCore, resultAuthContext.visit.MemberID, new List<string>());
										if (testUser != null)
										{
							...
							if (resultAuthContext.visit.id == 0)
							{
								...
								if (resultAuthContext.visitor.ID == 0)
								{
									...
									if (cpCore.siteProperties.getBoolean("AllowAutoRecognize", true))
									{
										...
										cpCore.security.decodeToken(CookieVisitor, cookieVisitorId, tokenDate);
										if (cookieVisitorId != 0)
										{
											//
											// -- visitor cookie good
											Models.Entity.visitorModel testVisitor = Models.Entity.visitorModel.create(cpCore, cookieVisitorId, new List<string>());
											if (testVisitor != null)
											{
												resultAuthContext.visitor = testVisitor;
								...
								if (resultAuthContext.visitor.ID == 0)
								{
									//
									// -- create new visitor
									resultAuthContext.visitor = Models.Entity.visitorModel.add(cpCore, new List<string>());
									visitor_changes = false;
									//
								...
								if (resultAuthContext.visitor.MemberID > 0)
								{
									...
									if (resultAuthContext.recognizeById(cpCore, resultAuthContext.visitor.MemberID, ref resultAuthContext))
									{
										...
										if (cpCore.siteProperties.AllowAutoLogin & resultAuthContext.user.AutoLogin & resultAuthContext.visit.CookieSupport)
										{
											...
											Models.Entity.visitModel lastVisit = Models.Entity.visitModel.getLastVisitByVisitor(cpCore, resultAuthContext.visit.id, resultAuthContext.visitor.ID);
											if (lastVisit != null)
											{
							...
							if (resultAuthContext.user.id < 1)
							{
								...
								else
								{
									if (resultAuthContext.visit.CookieSupport)
									{
										...
										resultAuthContext.user = Models.Entity.personModel.add(cpCore, new List<string>());
										user_changes = true;
		...
		public void logout(coreClass cpCore)
		{
			try
			{
				...
				//
				user = Models.Entity.personModel.add(cpCore, new List<string>());
				//
		...
		public bool recognizeById(coreClass cpCore, int userId, ref authContextModel authContext)
		{
			...
			try
			{
				...
				{
					authContext.visitor = Models.Entity.visitorModel.add(cpCore, new List<string>());
				}
				if (authContext.visit.id == 0)
				{
					authContext.visit = Models.Entity.visitModel.add(cpCore, new List<string>());
				}
				authContext.user = Models.Entity.personModel.create(cpCore, userId, new List<string>());
		...
		public bool isAuthenticatedMember(coreClass cpCore)
		{
			...
			try
			{
				result = visit.VisitAuthenticated & (Models.Complex.cdefModel.isWithinContent(cpCore, user.ContentControlID, Models.Complex.cdefModel.getContentId(cpCore, "members")));
