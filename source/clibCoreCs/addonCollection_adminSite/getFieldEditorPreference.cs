			...
			try
			{
				...
				if (dt.Rows.Count > 0)
				{
					foreach (DataRow rsDr in dt.Rows)
					{
						...
						{
							result = result + Environment.NewLine + "\t" + "<div class=\"radioCon\">" + cpCore.html.html_GetFormInputRadioBox(radioGroupName, genericController.encodeText(addonId), currentEditorAddonId.ToString()) + "&nbsp;Use " + genericController.encodeText(rsDr("addonName")) + "</div>"
													+ "";
						}
				...
				result = ""
										...
										+ Environment.NewLine + "\t" + "<div class=\"radioCon\">" + cpCore.html.html_GetFormInputRadioBox("setEditorPreference" + fieldId, "0", "0") + "&nbsp;Use Default Editor" + addonDefaultEditorName + "</div>"
