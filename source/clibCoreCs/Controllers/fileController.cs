	...
	public class fileController : IDisposable
	{
		...
		public void appendFile(string PathFilename, string FileContent)
		{
			try
			{
				...
				else
				{
					...
					{
						using (IO.StreamWriter sw = IO.File.CreateText(absFile))
						{
					...
					{
						using (IO.StreamWriter sw = IO.File.AppendText(absFile))
						{
							sw.Write(FileContent);
						}
					}
		...
		public IO.DirectoryInfo[] getFolderList(string FolderPath)
		{
			...
			try
			{
				if (!isLocal)
				{
					returnFolders = new IO.DirectoryInfo[0];
				}
				else
				{
					if (!pathExists(FolderPath))
		...
		public double getDriveFreeSpace()
		{
			...
			try
			{
				IO.DriveInfo scriptingDrive = null;
				string driveLetter;
				//
				// Drive Space
				//
