    ...
    public class securityController {
        ...
        private class hashEncode {
            ...
            public static string ComputeHash(string plainText, string hashAlgorithm, byte[] saltBytes) {
                ...
                byte[] plainTextWithSaltBytes = new byte[] {
                        (plainTextBytes.Length 
                                    + (saltBytes.Length - 1))};
                int I;
                for (I = 0; (I 
                            <= (plainTextBytes.Length - 1)); I++) {
                    plainTextWithSaltBytes[I] = plainTextBytes[I];
                }
                
                //  Append salt bytes to the resulting array.
                for (I = 0; (I 
                            <= (saltBytes.Length - 1)); I++) {
                ...
                byte[] hashWithSaltBytes = new byte[] {
                        (hashBytes.Length 
                                    + (saltBytes.Length - 1))};
                for (I = 0; (I 
                            <= (hashBytes.Length - 1)); I++) {
            ...
            public static bool VerifyHash(string plainText, string hashAlgorithm, string hashValue) {
                ...
                hashSizeInBytes = int.Parse((hashSizeInBits / 8));
                ...
                expectedHashString = hashEncode.ComputeHash(plainText, hashAlgorithm, saltBytes);
        ...
        public string encodeToken(int keyInteger, DateTime keyDate) {
            ...
            try {
                ...
                returnToken = this.twoWayEncrypt(sourceText);
            }
            catch (Exception ex) {
                cpCore.handleException(ex, ("EncodeToken failure. Returning blank result for keyInteger [" 
                                + (keyInteger + ("], keyDate [" 
                                + (keyDate + "]")))));
                returnToken = "";
            }
        ...
        public void decodeToken(string token, ref int returnNumber, ref DateTime returnDate) {
            try {
                ...
                string[] parts;
                decodedString = this.twoWayDecrypt(token);
                parts = decodedString.Split(((char)('\t')));
                if ((parts.Length == 2)) {
                    // 
                ...
                cpCore.handleException(ex, ("DecodeToken failure. Returning blank result for token [" 
                                + (token + "]")));
                returnNumber = 0;
                MinValue;
            }
