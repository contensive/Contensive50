    ...
    Public Class workerClass
                        ...
                        Else
                            ...
                            If True Then
                                ...
                                If True Then
                                    ...
                                    maxCmdInstances = cpCore.cluster.config.maxCmdInstances
        ...
        Private Function tcpListenerConnectionThread_executeCmd(cpCore As cpCoreClass, ByVal Method As String, ByVal queryString As String, ByVal RemoteIP As String) As String
            ...
            Try
                ...
                '
                cpCore.appendLog("serverClass.executeServerCmd, method=[" & Method & "], queryString=[" & queryString & "]")
                '
                ...
                If (Not IsLocalRemote) And ((RemoteUsername = "") Or (RemoteUsername <> AdminUsername) Or (RemotePassword <> AdminPassword)) Then
                    ...
                    '
                    cpCore.appendLog("serverClass.executeServerCmd", "bad username/password.")
                    returnString = "ERROR " & ccError_InvalidAuthentication
                Else
                    '
                    ' authenticated
                    '
                    cpCore.appendLog("serverClass.executeServerCmd, switch on method=[" & Method & "]")
                    Select Case UCase(Method)
                        ...
                        Case "START", "STARTALL", "UPGRADE", "UPGRADEALL", "EXPORTCDEF", "IMPORTCDEF", "RESTART", "STOP", "STOPALL", "PAUSE", "PAUSEALL", "RESUME", "RESUMEALL", "INSTALLADDONS", "INSTALLADDON", "IISRESET", "UNZIPANDDELETEFILE", "UNZIPFILE", "ZIPFILE", "REGISTERACTIVEX", "REGISTERDOTNET", "ADDSITE", "IMPORTSITE", "RUNPROCESS"
                            ...
                            '
                            cpCore.appendLog("serverClass.executeServerCmd, ccCMD method case")
                            '
                            Cmd = Method
                            ...
                            End If
                            '
                            cpCore.appendLog("serverClass.executeServerCmd, adding command to Queue [" & Cmd & "]")
                            '
                        ...
                        Case Else
                            ...
                            '
                            cpCore.appendLog("serverClass.executeServerCmd, unknown cmd=[" & Cmd & "]")
                            '
                    End Select
                End If
            Catch ex As Exception
        ...
        Private Sub RunSiteProcesses()
            ...
            Try
                ...
                hint &= ",enumerate appService in KernelService"
                ' !!!!! THIS RUNS THROUGH ALL SITES INTHE CLUSTER
                clusterAppList = cpCore.cluster.getAppNameList
                For Each AppName In clusterAppList
                    '
                    ...
                    If cpSite.core.app.status = applicationStatusEnum.ApplicationStatusReady Then
                        ...
                        Catch ex As Exception
                            ...
                            'If CSConnection.ConnectionHandle <> -1 Then
...
#End If
        ...
        Private Function addAsyncCmd(cpCore As cpCoreClass, ByVal Command As String, ByVal BlockDuplicates As Boolean) As Boolean
            ...
            Try
                ...
                '
                cpCore.appendLog("serverClass.addAsyncCmd, command=[" & Command & "], BlockDuplicates=[" & BlockDuplicates & "]")
                '
                returnBoolean = True
                ...
                If asyncCmdQueueCnt >= asyncCmdQueueLimit Then
                    ...
                    '
                    returnBoolean = False
                    cpCore.appendLog("addAsyncCmd, Server Cmd was blocked because Server Queue is too long [" & asyncCmdQueueCnt & "], command was [" & Command & "]")
                ElseIf BlockDuplicates Then
                    ...
                    For Ptr = 0 To asyncCmdQueueCnt - 1
                        If LCase(asyncCmdQueue(Ptr)) = LcaseCommand Then
                            returnBoolean = False
                            cpCore.appendLog("addAsyncCmd, Server Cmd was blocked because there is a duplicate in the queue already, [" & Command & "]")
                            Exit For
                        End If
                    Next
                End If
                ...
                If returnBoolean Then
                    ...
                    '
                    cpCore.appendLog("serverClass.addAsyncCmd, command added to ServerCmds, index=[" & asyncCmdQueueCnt & "], call runProcess...")
                    '
                    Call runProcess(cpCore, getProgramFilesPath() & "\ccCmd.exe", "port=" & serverListenerPort & " max=" & maxCmdInstances)
                End If
                '
                cpCore.appendLog("serverClass.addAsyncCmd, exit")
                '
            Catch ex As Exception
