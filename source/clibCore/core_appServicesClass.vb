...
Namespace Contensive.Core
    Public Class appServicesClass
        ...
        '------------------------------------------------------------------------------------------------------------------------
        '
        Friend ReadOnly Property metaData As metaDataClass
            Get
                If _metaData Is Nothing Then
                    _metaData = New metaDataClass(cpCore)
                End If
                Return _metaData
            End Get
        End Property
        Private _metaData As metaDataClass = Nothing
        '
        '------------------------------------------------------------------------------------------------------------------------
        ' internal storage
        ...
        ' file systems
        '
        Public Property serverFiles As fileSystemClass           ' files written directly to the local server
        Public Property appRootFiles As fileSystemClass         ' wwwRoot path for the app server, both local and scale modes
        Public Property privateFiles As fileSystemClass         ' path not available to web interface, local: physcial storage location, scale mode mirror location
        Public Property cdnFiles As fileSystemClass             ' file uploads etc. Local mode this should point to appRoot folder (or a virtual folder in appRoot). Scale mode it goes to an s3 mirror
        '
        '------------------------------------------------------------------------------------------------------------------------
            ...
            If Not Me.disposed Then
                If disposing Then
                    ...
                    If Not (_metaData Is Nothing) Then
                        _metaData.Dispose()
                    End If
                    If Not (_addonInstall Is Nothing) Then
                        '_addonInstall.dispose()
                    End If
                    '
...
ErrorTrap:
                        ...
                        If returnTagInvalidationDate = Date.MinValue Then
                            ...
                            Try
                                sql = "select top 1 t.modifiedDate" _
                                    ...
                                    & " where c.name=" & EncodeSQLText(tag) _
        ...
        '========================================================================
        '
        Public Sub cache_invalidateTag(ByVal tag As String)
            Try
                ...
                If config.enableCache Then
                    If tag <> "" Then
                        ...
                        '
                        cache_saveRaw(cacheName & tag, Now.ToString)
                        '
                        ' test if this tag is a content name
                        '
                        cdef = metaData.getCdef(tag)
                        If Not cdef Is Nothing Then
                            ...
                            If cdef.childIdList.Count > 0 Then
                                For Each childId As Integer In cdef.childIdList
                                    Dim childCdef As metaDataClass.CDefClass
                                    childCdef = metaData.getCdef(childId)
                                    If Not childCdef Is Nothing Then
                                        cache_saveRaw(cacheName & childCdef.Name, Now.ToString)
                                    End If
                                Next
                            End If
                            '
                            ...
                            Do While cdef.parentID > 0
                                ...
                                cdef = metaData.getCdef(cdef.parentID)
                                If Not cdef Is Nothing Then
                                    cache_saveRaw(cacheName & cdef.Name, Now.ToString)
                                End If
                            Loop
                        End If
                    End If
        ...
        '========================================================================
        '
        Public Sub cache_invalidateTagList(ByVal tagList As String)
            Try
                Dim tags() As String
                Dim tag As String
...
ErrorTrap:
        ...
        '=========================================================================================
        '
        Public Function AbuseCheck(ByVal AbuseKey As String) As Boolean
            Return False
            Exit Function
            '            On Error GoTo ErrorTrap

            '            'Const Tn = "AppServicesClass.AbuseCheck" : Call logMethodEntry(Tn) : ''Dim th as integer : th = TimerTraceStart(Tn)

            '            '
            '            Dim Traffic() As String
            '            Dim TrafficCount as integer
            '            Dim TrafficPointer as integer
            '            Dim NowTime As Double
            '            Dim TrafficSplit() As String
            '            Dim HitPointer as integer
            '            Dim HitCount as integer
            '            Dim NewAbuseCheckBuffer As String
            '            Dim NewEntry As String
            '            Dim TargetFound As Boolean
            '            Dim LimitTimeString As String
            '            Dim BufferLine As String
            '            '
            '            If AbuseCheckPeriod = 0 Then
            '                AbuseCheckPeriod = 10
            '            End If
            '            If AbuseCheckLimit = 0 Then
            '                AbuseCheckLimit = 10
            '            End If
            '            '
            '            AbuseCheck = False
            '            NowTime = Int(CDbl(Now) * 86400)
            '            LimitTimeString = CStr(NowTime - AbuseCheckPeriod)
            '            '
            '            If AbuseCheckBuffer = "" Then
            '                NewAbuseCheckBuffer = vbCrLf & AbuseKey & " " & NowTime
            '            Else
            '                Traffic = Split(AbuseCheckBuffer, vbCrLf)
            '                TrafficCount = UBound(Traffic) + 1
            '                For TrafficPointer = 1 To TrafficCount - 1
            '                    TrafficSplit = Split(Traffic(TrafficPointer), " ")
            '                    HitCount = UBound(TrafficSplit)
            '                    BufferLine = ""
            '                    If HitCount > 0 Then
            '                        For HitPointer = 1 To UBound(TrafficSplit)
            '                            If TrafficSplit(HitPointer) < LimitTimeString Then
            '                                '
            '                                ' Old hit, drop it
            '                                '
            '                                HitCount = HitCount - 1
            ...
            '                                ' Keep it
            '                                '
            '                                BufferLine = BufferLine & " " & TrafficSplit(HitPointer)
            '                            End If
            '                            '''DoEvents()
            '                        Next
            '                    End If
            '                    If (TrafficSplit(0) = AbuseKey) And Not TargetFound Then
            '                        '
            '                        ' Traffic is the target visit
            '                        '
            '                        TargetFound = True
            '                        BufferLine = BufferLine & " " & CStr(NowTime)
            '                        HitCount = HitCount + 1
            '                        AbuseCheck = (HitCount > AbuseCheckLimit)
            '                    End If
            '                    If (HitCount > 0) And (BufferLine <> "") Then
            '                        '
            '                        ' Add this visit back into the Abuse Check buffer
            '                        '
            '                        NewAbuseCheckBuffer = NewAbuseCheckBuffer & vbCrLf & TrafficSplit(0) & BufferLine
            '                    End If
            '                    '''DoEvents()
            '                Next
            '                If Not TargetFound Then
            '                    '
            '                    ' Target is a new entry
            '                    '
            '                    NewAbuseCheckBuffer = NewAbuseCheckBuffer & vbCrLf & AbuseKey & " " & NowTime
            '                End If
            '            End If
            '            AbuseCheckBuffer = NewAbuseCheckBuffer
            ...
            '            '
            'ErrorTrap:
            '            Call HandleClassTrapError("AbuseCheck", "Error during abuse check.")
            '            'LoadContentEngineInProcess = True
        End Function
        ...
        End Property
        '        '
        '        ' Returns the list of field names from a content definition
        '        '
        '        Public Function GetContentFieldList(ByVal ContentName As String) As String
        '            On Error GoTo ErrorTrap

        '            'Const Tn = "AppServicesClass.GetContentFieldList" : Call logMethodEntry(Tn) : ''Dim th as integer : th = TimerTraceStart(Tn)

        '            '
        '            Dim ContentPointer As Integer
        '            '
        '            GetContentFieldList = ""
        '            ContentPointer = metaData.metaCache.cdefContentNameIndex.getPtr(Trim(ContentName))
        '            If (ContentPointer >= 0) Then
        '                GetContentFieldList = metaData.metaCache.cdef(ContentPointer).SelectFieldList
        '            End If
        '            '
        ...
        '            '
        'ErrorTrap:
        '            Call handleLegacyClassError1(config.name, "GetContentFieldList(" & ContentName & ")")
        '        End Function
        '        '
        '        '========================================================================================================
        '        '   getContentNameIndex
        '        '
        '        '   returns the keyPtrIndex property bag of the object created during loadcontentdefinitioins
        '        '========================================================================================================
        '        '
        '        Public Function getContentNameIndex() As Object
        '            On Error GoTo ErrorTrap

        '            'Const Tn = "AppServicesClass.getContentNameIndex" : Call logMethodEntry(Tn) : ''Dim th as integer : th = TimerTraceStart(Tn)

        '            '
        '            getContentNameIndex = metaData.metaCache.cdefContentNameIndex.exportPropertyBag
        '            '
        ...
        '            '
        'ErrorTrap:
        '            Call handleLegacyClassError1(config.name, "unknown")
        '        End Function
        '        '
        '        '========================================================================================================
        '        '   getContentIdIndex
        ...
        '        '========================================================================================================
        '        '
        '        Public Function getContentIdIndex() As Object
        '            On Error GoTo ErrorTrap

        '            'Const Tn = "AppServicesClass.getContentIdIndex" : Call logMethodEntry(Tn) : ''Dim th as integer : th = TimerTraceStart(Tn)

        '            '
        '            getContentIdIndex = metaData.metaCache.cdefContentIDIndex.exportPropertyBag
        '            '
        ...
        '
        '
        'Private Sub logMethodEntry(ByVal MethodName As String)
        '    tabcnt = tabcnt + 1
        '    cpCore.appendLog("", "", "")
        '    cpCore.appendLog("thread=" & Threading.Thread.CurrentThread.ManagedThreadId.ToString & New String(vbTab, tabcnt) & MethodName & " enter")
        'End Sub
        '
        '
        '
        'Private Sub logMethodExit(ByVal MethodName)
        '    If tabcnt < 1 Then
        '        tabcnt = 1
        '        cpCore.appendLog("******* tbcnt was negative")
        '    End If
        '    cpCore.appendLog("thread=" & Threading.Thread.CurrentThread.ManagedThreadId.ToString & New String(vbTab, tabcnt) & MethodName & " exit")
        '    tabcnt = tabcnt - 1
        'End Sub
        '
        '
            ...
            Try
                If config.enableCache Then
                    If Not (cpCore Is Nothing) Then
                        returnObj = cpCore.cluster.cache_readRaw(config.name & "-" & key)
                    End If
                End If
            Catch ex As Exception
        ...
        Public Sub cache_saveRaw(ByVal Key As String, ByVal data As Object, Optional invalidationDate As Date = #12:00:00 AM#)
            Try
                If config.enableCache Then
                    ...
                    End If
                    Call cpCore.cluster.cache_saveRaw(config.name & "-" & Key, data, invalidationDate)
                End If
            Catch ex As Exception
                Call cpCore.handleLegacyError8("exception")
            End Try
        ...
        '========================================================================
        '
        Public Sub cache_invalidateAll()
            Try
                If config.enableCache Then
                    cache_globalInvalidationDate = Now
                    Call cache_saveRaw("globalInvalidationDate", cache_globalInvalidationDate, Now.AddYears(1))
                    '
                    ...
                    Call cpCore.cache_pageContent_clear()
                    Call cpCore.cache_pageTemplate_clear()
                    Call cpCore.cache_siteSection_clear()
                    If Not IsNothing(cpCore.cache_addonStyleRules) Then
                        cpCore.cache_addonStyleRules.clear()
                    End If
                    '
                    ' refactor - dont know why this is here, but it should not be necessayr since the global sill invalidate them all 
                    If False Then
                        CS = db_csOpen("Content", , , , , , , "name")
                        Do While db_csOk(CS)
                            Call cache_invalidateTag(db_GetCSText(CS, "name"))
                            Call db_csGoNext(CS)
                        Loop
                        Call db_csClose(CS)
                    End If
                    '
                    Call privateFiles.DeleteFileFolder("AppCache")
                    Call privateFiles.createPath("AppCache")
                End If
            Catch ex As Exception
                cpCore.handleException(ex)
            End Try
        ...
        '========================================================================
        '
        Public Function cache_read(Of resultType)(ByVal Name As String) As Object
            ...
            Try
                ...
                If config.enableCache Then
                    If (siteProperty_AllowCache_SpecialCaseNotCached) Then
                        ...
                        '
                        If (Name <> "") Then
                            cacheData = DirectCast(cache_readRaw(Name), CacheDataClass)
                            If Not (cacheData Is Nothing) Then
                                If cache_globalInvalidationDate < cacheData.saveDate Then
                                    ...
                                    '
                                    For Each dataSourceTag As String In cacheData.dataSourceTagList
                                        If (cache_getTagInvalidationDate(dataSourceTag) > cacheData.saveDate) Then
                                            invalidate = True
                                            Exit For
                                        End If
                                    Next
        ...
        '========================================================================
        '
        Public Sub cache_save(ByVal key As String, ByVal data As Object, Optional ByVal tagList As String = "", Optional ByVal invalidationDate As Date = #12:00:00 AM#)
            Try
                ...
                If config.enableCache Then
                    If (siteProperty_AllowCache_SpecialCaseNotCached) Then
                        ...
                        If Not String.IsNullOrEmpty(tagList) Then
                            cacheData.dataSourceTagList.AddRange(tagList.Split(CChar(",")))
                        End If
                        cacheData.saveDate = Now()
                        Call cache_saveRaw(key, cacheData, invalidationDate)
                    End If
                End If
            Catch ex As Exception
                cpCore.handleException(ex)
        ...
        Public Sub siteProperty_set(ByVal propertyName As String, ByVal Value As String)
            Try
                ...
                Dim SQLNow As String = EncodeSQLDate(Now)

                RecordID = 0
                SQL = "SELECT ID FROM CCSETUP WHERE NAME=" & EncodeSQLText(propertyName) & " order by id"
                dt = executeSql(SQL)
                If dt.Rows.Count > 0 Then
                    RecordID = EncodeInteger(dt.Rows(0).Item("ID"))
                End If
                If RecordID <> 0 Then
                    SQL = "UPDATE ccSetup Set FieldValue=" & EncodeSQLText(Value) & ",ModifiedDate=" & SQLNow & " WHERE ID=" & RecordID
                    Call executeSql(SQL)
                Else
                    ...
                    SQL = "INSERT INTO ccSetup (ACTIVE,CONTENTCONTROLID,NAME,FIELDVALUE,ModifiedDate,DateAdded)VALUES(" & SQLTrue & "," & EncodeSQLNumber(ContentID) & "," & EncodeSQLText(UCase(propertyName)) & "," & EncodeSQLText(Value) & "," & SQLNow & "," & SQLNow & ");"
                    Call executeSql(SQL)
                End If
                Call cache_save(cacheName, Value, "site properties", Now.AddDays(1))
        ...
        Public Function siteProperty_getText_noCache(ByVal PropertyName As String, ByVal DefaultValue As String, ByRef return_propertyFound As Boolean) As String
            ...
            Try
                ...
                SQL = "select FieldValue from ccSetup where name=" & EncodeSQLText(PropertyName) & " order by id"
                dt = executeSql(SQL)
                If dt.Rows.Count > 0 Then
        ...
        Public Function siteProperty_getText(ByVal PropertyName As String, Optional ByVal DefaultValue As String = "") As String
            ...
            Try
                ...
                returnString = EncodeText(cache_read(Of String)(cacheName))
                If returnString = "" Then
                    ...
                    If (propertyFound) And (returnString <> "") Then
                        Call cache_save(cacheName, returnString, "Site Properties", Now.AddDays(1))
                    End If
                End If
            Catch ex As Exception
                cpCore.handleException(ex)
...
ErrorTrap:
        ...
        Public Function csv_IsSQLTableField(ByVal DataSourceName As String, ByVal TableName As String, ByVal FieldName As String) As Boolean
            ...
            Try
                Dim tableSchema As metaDataClass.tableSchemaClass
                '
...
ErrorTrap:
        ...
        Public Sub csv_CreateSQLIndex(ByVal DataSourceName As String, ByVal TableName As String, ByVal IndexName As String, ByVal FieldNames As String, Optional clearMetaCache As Boolean = False)
            Try
                Dim ts As metaDataClass.tableSchemaClass
...
ErrorTrap:
        ...
        Public Sub csv_DeleteCSRecord(ByVal CSPointer As Integer)
            ...
            Dim fieldArray As metaDataClass.CDefFieldClass()
                ...
                Else
                    ...
                    If Not csv_ContentSet(CSPointer).WorkflowAuthoringMode Then
                        ...
                        If csv_AllowAutocsv_ClearContentTimeStamp Then
                            Call cache_invalidateTag(ContentName)
                        End If
                    ...
                    Else
                        ...
                        Call sqlList.add("MODIFIEDBY", EncodeSQLNumber(csv_ContentSet(CSPointer).OwnerMemberID)) ' Pointer)
...
ErrorTrap:
        ...
        '========================================================================
        '
        Public Function db_GetCSField(ByVal CSPointer As Integer, ByVal FieldName As String) As Object
                ...
                With csv_ContentSet(CSPointer)
                    ...
                    If Not fieldFound Then
                        ...
                        If useCSReadCacheMultiRow Then
                            ...
                            Else
                                db_GetCSField = .dt.Rows(.readCacheRowPtr).Item(FieldName.ToLower)
                            End If
                        Else
                            '
...
ErrorTrap:
        ...
        Public Function db_GetCSFieldTypeId(ByVal CSPointer As Integer, ByVal FieldName As String) As Integer
            ...
            Dim ContentName As String
            'Dim CDef As CDefType
            Dim arrayOfFields As metaDataClass.CDefFieldClass()
            '
...
ErrorTrap:
                ...
                Else
                    With CDef
                        ...
                        If .fields.Count > 0 Then
                            For Each keyValuePair As KeyValuePair(Of String, metaDataClass.CDefFieldClass) In .fields
                                        ...
                                        Select Case UCase(FieldName)
                                            ...
                                            Case Else
                                                ...
                                                Select Case .fieldTypeId
                                                    ...
                                                    '
                                                    Case FieldTypeIdBoolean
                                                        sqlList.add(FieldName, EncodeSQLBoolean(.defaultValue))
                                                    ...
                                                    Case FieldTypeIdDate
                                                        sqlList.add(FieldName, EncodeSQLDate(.defaultValue))
        ...
        Public Function db_OpenCSContentRecord(ByVal ContentName As String, ByVal RecordID As Integer, Optional ByVal MemberID As Integer = SystemMemberID, Optional ByVal WorkflowAuthoringMode As Boolean = False, Optional ByVal WorkflowEditingMode As Boolean = False, Optional ByVal SelectFieldList As String = "") As Integer
            ...
            '
            db_OpenCSContentRecord = db_csOpen(ContentName, "(ID=" & EncodeSQLNumber(RecordID) & ")", , False, MemberID, WorkflowAuthoringMode, WorkflowEditingMode, SelectFieldList, 1)
            '
            Exit Function
            '
ErrorTrap:
...
ErrorTrap:
        ...
        Public Sub db_CopyCSRecord(ByVal CSSource As Integer, ByVal CSDestination As Integer)
            ...
            Dim DestCDef As metaDataClass.CDefClass
            '
                                            ...
                                            Case FieldTypeIdLookup
                                                ...
                                                If IsNumeric(FieldValueVariant) Then
                                                    ...
                                                    If (LookupContentName <> "") Then
                                                        ...
                                                        '
                                                        CSLookup = db_csOpen(LookupContentName, "ID=" & EncodeSQLNumber(FieldValueVariant), , , , , , "name", 1)
                    ...
                    If (.Updateable) And (.writeCache.Count > 0) Then
                        ...
                        For Each keyValuePair In .writeCache
                            ...
                            Else
                                ...
                                '                            End If
                                '
                                Dim field As metaDataClass.CDefFieldClass = .CDef.fields(FieldName.ToLower())
                                If True Then
                                    ...
                                    With field
                                        ...
                                        Select Case .fieldTypeId
                                            ...
                                            Case FieldTypeIdBoolean
                                                SQLSetPair = FieldName & "=" & EncodeSQLBoolean(writeCacheValueVariant)
                                            Case FieldTypeIdDate
                                                SQLSetPair = FieldName & "=" & EncodeSQLDate(writeCacheValueVariant)
                                            Case FieldTypeIdText
                                            ...
                                            Case FieldTypeIdLink, FieldTypeIdResourceLink, FieldTypeIdFile, FieldTypeIdImage, FieldTypeIdTextFile, FieldTypeIdCSSFile, FieldTypeIdXMLFile, FieldTypeIdJavascriptFile, FieldTypeIdHTMLFile
                                                ...
                                                SQLSetPair = FieldName & "=" & EncodeSQLText(Copy)
                                            Case FieldTypeIdLongText, FieldTypeIdHTML
                                                SQLSetPair = FieldName & "=" & EncodeSQLLongText(writeCacheValueVariant)
                                            Case Else
                                                '
                                                ' Invalid fieldtype
                                                '
                        ...
                        ElseIf (FieldFoundCount > 0) Then
                            ...
                            If AuthorableFieldUpdate And (Not WorkflowRenderingMode) Then
                                ...
                                If csv_AllowAutocsv_ClearContentTimeStamp And (Not Blockcsv_ClearBake) Then
                                    Call cache_invalidateTag(LiveRecordContentName)
                                End If
                                '
        ...
        Public Function csv_OpenCSGroupUsers(ByVal GroupName As String, ByVal IsList As Boolean, Optional ByVal sqlCriteria As String = "", Optional ByVal SortFieldList As String = "", Optional ByVal ActiveOnly As Boolean = True, Optional ByVal PageSize As Integer = 9999, Optional ByVal PageNumber As Integer = 1) As Integer
            ...
            If GroupName <> "" Then
                ...
                If Not IsList Then
                    ...
                    SQL &= "" _
                        & "AND(ccGroups.Name=" & EncodeSQLText(GroupName) & ")" _
                        & ""
                Else
                    '
                    ' list of names
                    '
                    SQL &= "" _
                        & "AND(ccGroups.Name In (" & Replace(EncodeSQLText(GroupName), ",", "','") & "))" _
                        & ""
                End If
                '
                ' group expiration
...
ErrorTrap:
        ...
        Public Function csv_IsRecordLocked(ByVal ContentName As String, ByVal RecordID As Integer, ByVal MemberID As Integer) As Boolean
            ...
            Dim CS As Integer
            '
            Criteria = csv_GetAuthoringControlCriteria(ContentName, RecordID) & "and(CreatedBy<>" & EncodeSQLNumber(MemberID) & ")"
...
ErrorTrap:
        ...
        Private Function csv_GetAuthoringControlCriteria(ByVal ContentName As String, ByVal RecordID As Integer) As String
            ...
            ElseIf ContentCnt = 1 Then
                ...
                ' One content record
                '
                csv_GetAuthoringControlCriteria = "(ContentID=" & EncodeSQLNumber(Mid(Criteria, 2)) & ")and(RecordID=" & EncodeSQLNumber(RecordID) & ")and((DateExpires>" & EncodeSQLDate(Now) & ")or(DateExpires is null))"
            Else
                '
                ' Multiple content records
                '
                csv_GetAuthoringControlCriteria = "(ContentID in (" & Mid(Criteria, 2) & "))and(RecordID=" & EncodeSQLNumber(RecordID) & ")and((DateExpires>" & EncodeSQLDate(Now) & ")or(DateExpires is null))"
            End If
            '
            Exit Function
            '
        ...
        Public Sub csv_ClearAuthoringControl(ByVal ContentName As String, ByVal RecordID As Integer, ByVal AuthoringControl As Integer, ByVal MemberID As Integer)
            ...
            Select Case AuthoringControl
                Case AuthoringControlsEditing
                    Call db_DeleteContentRecords("Authoring Controls", Criteria & "and(CreatedBy=" & EncodeSQLNumber(MemberID) & ")", MemberID)
                ...
                Select Case AuthoringControl
                    Case AuthoringControlsEditing
                        ...
                        If (EditLockTimeoutMinutes <> 0) Then
                            ...
                            '
                            Call db_DeleteContentRecords("Authoring Controls", sqlCriteria & "AND(DATEEXPIRES<" & EncodeSQLDate(Now) & ")")
                            '
...
ErrorTrap:
        ...
        Public Sub csv_CreateContentChild(ByVal ChildContentName As String, ByVal ParentContentName As String, ByVal MemberID As Integer)
            ...
            '
            SQL = "select ID from ccContent where name=" & EncodeSQLText(ChildContentName) & ";"
            rs = executeSql(SQL)
            If isDataTableOk(rs) Then
            ...
            If ChildContentID = 0 Then
                ...
                '
                SQL = "select ID from ccContent where name=" & EncodeSQLText(ParentContentName) & ";"
                rs = executeSql(SQL, DataSourceName)
                If isDataTableOk(rs) Then
...
ErrorTrap:
        ...
        Public Function csv_OpenCSJoin(ByVal CSPointer As Integer, ByVal FieldName As String) As Integer
            ...
            Dim MethodName As String
            Dim CDef As metaDataClass.CDefClass
            Dim CDefLookup As metaDataClass.CDefClass
            '
                ...
                Else
                    ...
                    Else
                        ...
                        ElseIf IsNumeric(FieldValueVariant) Then
                            ...
                            Else
                                LookupContentName = csv_GetContentNameByID(fieldLookupId)
                                csv_OpenCSJoin = db_csOpen(LookupContentName, "ID=" & EncodeSQLNumber(FieldValueVariant), "name", , , , , , 1)
        ...
        Public Sub csv_PublishEdit(ByVal ContentName As String, ByVal RecordID As Integer, ByVal MemberID As Integer)
            ...
            If CDef.Id > 0 Then
                If CDef.AllowWorkflowAuthoring And siteProperty_AllowWorkflowAuthoring Then
                    ...
                    '
                    RSLive = executeSql("SELECT " & FieldList & " FROM " & LiveTableName & " WHERE ID=" & EncodeSQLNumber(LiveRecordID) & ";", LiveDataSourceName)
                    ...
                    Else
                        If True Then
                            ...
                            '
                            RSEdit = executeSql("SELECT " & FieldList & " FROM " & EditTableName & " WHERE (EditSourceID=" & EncodeSQLNumber(LiveRecordID) & ")and(EditArchive=0) Order By ID DESC;", EditDataSourceName)
                            ...
                            Else
                                If True Then
                                    ...
                                    If True Then
                                        ...
                                        PublishFieldNameArray(FieldPointer) = FieldName
                                        If PublishingDelete Then
                                            PublishSqlFieldList.add(FieldName, EncodeSQLNumber(0))
                                        Else
                                            PublishSqlFieldList.add(FieldName, EditSQLValue)
                                        End If
                                        FieldPointer = FieldPointer + 1
                                        ...
                                        NewEditSqlFieldList.add(FieldName, EncodeSQLNumber(LiveRecordID))
                                        ArchiveSqlFieldList.add(FieldName, EncodeSQLNumber(LiveRecordID))
                                        FieldPointer = FieldPointer + 1
                                        '
                                        FieldNameArray(FieldPointer) = "EDITARCHIVE"
                                        NewEditSqlFieldList.add(FieldName, EncodeSQLBoolean(False))
                                        ArchiveSqlFieldList.add(FieldName, EncodeSQLBoolean(True))
                                        '
        ...
        Public Sub csv_AbortEdit(ByVal ContentName As String, ByVal RecordID As Integer, ByVal MemberID As Integer)
            ...
            If CDef.Id > 0 Then
                If CDef.AllowWorkflowAuthoring And siteProperty_AllowWorkflowAuthoring Then
                    ...
                    ' Open the live record
                    '
                    RSLive = executeSql("SELECT * FROM " & LiveTableName & " WHERE ID=" & EncodeSQLNumber(LiveRecordID) & ";", LiveDataSourceName)
                    ...
                    Else
                        ...
                        Else
                            ...
                            ' Open the edit record
                            '
                            RSEdit = executeSql("SELECT * FROM " & EditTableName & " WHERE (EditSourceID=" & EncodeSQLNumber(LiveRecordID) & ")and(EditArchive=0) Order By ID DESC;", EditDataSourceName)
...
ErrorTrap:
        ...
        Public Sub csv_SetCSRecordDefaults(ByVal CS As Integer)
            ...
            Dim DefaultValueText As String
            '
            For Each keyValuePair As KeyValuePair(Of String, metaDataClass.CDefFieldClass) In csv_ContentSet(CS).CDef.fields
                Dim field As metaDataClass.CDefFieldClass = keyValuePair.Value
                With field
                    FieldName = .nameLc
                    If (FieldName <> "") And (Not String.IsNullOrEmpty(.defaultValue)) Then
...
ErrorTrap:
        ...
        Public Sub exportApplicationCDefXml(ByVal privateFilesPathFilename As String, ByVal IncludeBaseFields As Boolean)
            ...
            '
            Dim MethodName As String
            Dim XML As xmlToolsClass
            Dim Content As String
            '
            MethodName = "csv_ExportXML2"
            '
            XML = New xmlToolsClass(cpCore)
            'Call XML.Init(Me)
...
ErrorTrap:
        ...
        Public ReadOnly Property siteProperty_AllowCache_SpecialCaseNotCached() As Boolean
            Get
                ...
                If Not siteProperty_AllowCache_LocalLoaded Then
                    siteProperty_AllowCache_Local = EncodeBoolean(siteProperty_getText_noCache("AllowBake", "0", propertyFound))
        ...
        Public Function checkHealth() As applicationStatusEnum
            ...
            Try
                ...
                '--------------------------------------------------------------------------
                '
                Dim ts As metaDataClass.tableSchemaClass = cpCore.app.metaData.getTableSchema("ccContent", "Default")
