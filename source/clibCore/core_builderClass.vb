...
Namespace Contensive.Core
    Public Class builderClass
        ...
        '
        ' ================
        Public Sub web_recycle(appName As String)
            Try
        ...
        '=============================================================================
        '
        Private Function GetContentID(ByVal ContentName As String) As Integer
        ...
        Private Sub VerifyRecord(ByVal ContentName As String, ByVal Name As String, Optional ByVal CodeFieldName As String = "", Optional ByVal Code As String = "", Optional ByVal InActive As Boolean = False)
            Try
                ...
                Active = Not InActive
                'Dim createkey As Integer = GetRandomInteger()
                Dim cdef As metaDataClass.CDefClass = cpCore.app.metaData.getCdef(ContentName)
                Dim tableName As String = cdef.ContentTableName
                Dim cid As Integer = cdef.Id
                '
                dt = cpCore.app.executeSql("SELECT ID FROM " & tableName & " WHERE NAME=" & EncodeSQLText(Name) & ";")
                If dt.Rows.Count = 0 Then
                    ...
                    sql2 = ") values (" & cid & ",0," & EncodeSQLBoolean(Active) & "," & EncodeSQLText(Name)
                    sql3 = ")"
        ...
        Private Function GetIDBYName(ByVal TableName As String, ByVal RecordName As String) As Integer
            ...
            Try
                ...
                Dim rs As DataTable
                '
                rs = cpCore.app.executeSql("Select ID from " & TableName & " where name=" & EncodeSQLText(RecordName))
                If isDataTableOk(rs) Then
        ...
        Private Sub VerifyState(ByVal Name As String, ByVal Abbreviation As String, ByVal SaleTax As Double, ByVal CountryID As Integer, ByVal FIPSState As String)
            Try
                ...
                '
                CS = cpCore.app.db_csOpen(ContentName, "name=" & EncodeSQLText(Name), , False)
        ...
        Private Sub VerifyCountry(ByVal Name As String, ByVal Abbreviation As String)
            Try
                ...
                Dim Active As Boolean
                '
                CS = cpCore.app.db_csOpen("Countries", "name=" & EncodeSQLText(Name))
                If Not cpCore.app.db_csOk(CS) Then
                    Call cpCore.app.db_csClose(CS)
        ...
        Friend Sub VerifyDefaultGroups()
            Try
                ...
                SQL = "Update ccContent Set EditorGroupID=" & EncodeSQLNumber(GroupID) & " where EditorGroupID is null;"
                Call cpCore.app.executeSql(SQL)
            Catch ex As Exception
                cpCore.handleException(ex)
            End Try
        ...
        Public Sub csv_VerifyMenuEntry(ByVal ParentName As String, ByVal EntryName As String, ByVal ContentName As String, ByVal LinkPage As String, ByVal SortOrder As String, ByVal AdminOnly As Boolean, ByVal DeveloperOnly As Boolean, ByVal NewWindow As Boolean, ByVal Active As Boolean, ByVal MenuContentName As String, ByVal AddonName As String)
            Try
                ...
                ParentID = 0
                If ParentName <> "" Then
                    CS = cpCore.app.db_csOpen(MenuContentName, "name=" & EncodeSQLText(ParentName), "ID", False, , , , "ID", 1)
                    If cpCore.app.db_csOk(CS) Then
                ...
                '
                CSEntry = cpCore.app.db_csOpen(MenuContentName, "(name=" & EncodeSQLText(EntryName) & ")", "ID", False, , , , SelectList)
                '
            ...
            Try
                ...
                Else
                    ...
                    Else
                        ...
                        '
                        SQL = "select ID,ccguid,IsBaseContent  from ccContent where (name=" & EncodeSQLText(ContentName) & ") order by id;"
                        dt = cpCore.app.executeSql(SQL)
                        If dt.Rows.Count > 0 Then
                        ...
                        If Not String.IsNullOrEmpty(ParentName) Then
                            SQL = "select id from ccContent where (name=" & EncodeSQLText(ParentName) & ") order by id;"
                            dt = cpCore.app.executeSql(SQL)
                            If dt.Rows.Count > 0 Then
                        ...
                        If (installedByCollectionGuid <> "") Then
                            SQL = "select id from ccAddonCollections where ccGuid=" & EncodeSQLText(installedByCollectionGuid)
                            dt = cpCore.app.executeSql(SQL)
                            If dt.Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            '
                            SQL = "SELECT ID from ccTables where (active<>0) and (name=" & EncodeSQLText(TableName) & ");"
                            dt = cpCore.app.executeSql(SQL)
                            If dt.Rows.Count <= 0 Then
                                '
                                ...
                                sqlList.add("name", EncodeSQLText(TableName))
                                sqlList.add("active", SQLTrue)
                                sqlList.add("DATASOURCEID", EncodeSQLNumber(DataSourceID))
                                sqlList.add("CONTENTCONTROLID", EncodeSQLNumber(cpCore.app.csv_GetContentID("Tables")))
                                '
                            ...
                            If iDefaultSortMethod = "" Then
                                DefaultSortMethodID = 0
                            Else
                                dt = cpCore.app.db_openTable("Default", "ccSortMethods", "(name=" & EncodeSQLText(iDefaultSortMethod) & ")and(active<>0)", "ID", "ID", 1, 1)
                                If dt.Rows.Count > 0 Then
                            ...
                            If DefaultSortMethodID = 0 Then
                                ...
                                '
                                dt = cpCore.app.db_openTable("Default", "ccSortMethods", "(OrderByClause=" & EncodeSQLText(iDefaultSortMethod) & ")and(active<>0)", "ID", "ID", 1, 1)
                                If dt.Rows.Count > 0 Then
                                    DefaultSortMethodID = EncodeInteger(dt.Rows(0).Item("ID"))
                                End If
                            End If
                            ...
                            '
                            sqlList = New sqlFieldListClass
                            Call sqlList.add("name", EncodeSQLText(ContentName))
                            Call sqlList.add("CREATEKEY", "0")
                            Call sqlList.add("active", EncodeSQLBoolean(Active))
                            Call sqlList.add("ContentControlID", EncodeSQLNumber(ContentIDofContent))
                            Call sqlList.add("AllowAdd", EncodeSQLBoolean(AllowAdd))
                            Call sqlList.add("AllowDelete", EncodeSQLBoolean(AllowDelete))
                            Call sqlList.add("AllowWorkflowAuthoring", EncodeSQLBoolean(AllowWorkflowAuthoring))
                            Call sqlList.add("DeveloperOnly", EncodeSQLBoolean(DeveloperOnly))
                            Call sqlList.add("AdminOnly", EncodeSQLBoolean(AdminOnly))
                            Call sqlList.add("ParentID", EncodeSQLNumber(parentId))
                            Call sqlList.add("DefaultSortMethodID", EncodeSQLNumber(DefaultSortMethodID))
                            Call sqlList.add("DropDownFieldList", EncodeSQLText(encodeEmptyText(DropDownFieldList, "Name")))
                            Call sqlList.add("ContentTableID", EncodeSQLNumber(TableID))
                            Call sqlList.add("AuthoringTableID", EncodeSQLNumber(TableID))
                            Call sqlList.add("ModifiedDate", EncodeSQLDate(Now))
                            Call sqlList.add("CreatedBy", EncodeSQLNumber(SystemMemberID))
                            Call sqlList.add("ModifiedBy", EncodeSQLNumber(SystemMemberID))
                            Call sqlList.add("AllowCalendarEvents", EncodeSQLBoolean(AllowCalendarEvents))
                            Call sqlList.add("AllowContentTracking", EncodeSQLBoolean(AllowContentTracking))
                            Call sqlList.add("AllowTopicRules", EncodeSQLBoolean(AllowTopicRules))
                            Call sqlList.add("AllowContentChildTool", EncodeSQLBoolean(AllowContentChildTool))
                            Call sqlList.add("AllowMetaContent", EncodeSQLBoolean(AllowMetaContent))
                            Call sqlList.add("IconLink", EncodeSQLText(encodeEmptyText(IconLink, "")))
                            Call sqlList.add("IconHeight", EncodeSQLNumber(IconHeight))
                            Call sqlList.add("IconWidth", EncodeSQLNumber(IconWidth))
                            Call sqlList.add("IconSprites", EncodeSQLNumber(IconSprites))
                            Call sqlList.add("installedByCollectionid", EncodeSQLNumber(InstalledByCollectionID))
                            If SupportsGuid Then
                                If (LcContentGuid = "") And (NewGuid <> "") Then
                                    ...
                                    '
                                    Call sqlList.add("ccGuid", EncodeSQLText(NewGuid))
                            ...
                            If parentId < 1 Then
                                ...
                                If Not cpCore.app.csv_isCdefField(ContentID, "ID") Then
                                    field = New metaDataClass.CDefFieldClass
                                    field.nameLc = "id"
                                    field.active = True
        ...
        Public Sub csv_CreateContentFieldFromTableField(ByVal ContentName As String, ByVal FieldName As String, ByVal ADOFieldType As Integer)
            Try
                '
                Dim field As New metaDataClass.CDefFieldClass
                '
        ...
        ' ====================================================================================================================
        '
        Public Function metaData_VerifyCDefField_ReturnID(ByVal ContentName As String, field As metaDataClass.CDefFieldClass) As Integer ' , ByVal FieldName As String, ByVal Args As String, ByVal Delimiter As String) As Integer
            Dim returnId As Integer = 0
            Try
                '
                ...
                ContentID = -1
                TableID = 0
                SQL = "select ID,ContentTableID from ccContent where name=" & EncodeSQLText(ContentName) & ";"
                rs = cpCore.app.executeSql(SQL)
                If isDataTableOk(rs) Then
                ...
                SQL = "select ID,IsBaseField from ccFields where (ContentID=" & EncodeSQLNumber(ContentID) & ")and(name=" & EncodeSQLText(field.nameLc) & ");"
                rs = cpCore.app.executeSql(SQL)
                If isDataTableOk(rs) Then
                ...
                If True Then
                    ...
                    Else
                        ...
                        '
                        TableName = ""
                        rs = cpCore.app.executeSql("Select Name, DataSourceID from ccTables where ID=" & EncodeSQLNumber(TableID) & ";")
                        If Not isDataTableOk(rs) Then
                        ...
                        If (TableName <> "") Then
                            ...
                            If (DataSourceID < 1) Then
                                DataSourceName = "Default"
                            Else
                                rs = cpCore.app.executeSql("Select Name from ccDataSources where ID=" & EncodeSQLNumber(DataSourceID) & ";")
                                If Not isDataTableOk(rs) Then
                            ...
                            InstalledByCollectionID = 0
                            If (installedByCollectionGuid <> "") Then
                                rs = cpCore.app.executeSql("Select id from ccAddonCollections where ccguid=" & EncodeSQLText(installedByCollectionGuid) & ";")
                                If isDataTableOk(rs) Then
                            ...
                            Pointer = 0
                            Call sqlList.add("ACTIVE", EncodeSQLBoolean(field.active)) ' Pointer)
                            Call sqlList.add("MODIFIEDBY", EncodeSQLNumber(SystemMemberID)) ' Pointer)
                            Call sqlList.add("MODIFIEDDATE", EncodeSQLDate(Now)) ' Pointer)
                            Call sqlList.add("TYPE", EncodeSQLNumber(fieldTypeId)) ' Pointer)
                            Call sqlList.add("CAPTION", EncodeSQLText(FieldCaption)) ' Pointer)
                            Call sqlList.add("ReadOnly", EncodeSQLBoolean(FieldReadOnly)) ' Pointer)
                            Call sqlList.add("LOOKUPCONTENTID", EncodeSQLNumber(LookupContentID)) ' Pointer)
                            Call sqlList.add("REQUIRED", EncodeSQLBoolean(FieldRequired)) ' Pointer)
                            Call sqlList.add("TEXTBUFFERED", SQLFalse) ' Pointer)
                            Call sqlList.add("PASSWORD", EncodeSQLBoolean(Password)) ' Pointer)
                            Call sqlList.add("EDITSORTPRIORITY", EncodeSQLNumber(field.editSortPriority)) ' Pointer)
                            Call sqlList.add("ADMINONLY", EncodeSQLBoolean(field.adminOnly)) ' Pointer)
                            Call sqlList.add("DEVELOPERONLY", EncodeSQLBoolean(FieldDeveloperOnly)) ' Pointer)
                            Call sqlList.add("CONTENTCONTROLID", EncodeSQLNumber(cpCore.app.csv_GetContentID("Content Fields"))) ' Pointer)
                            Call sqlList.add("DefaultValue", EncodeSQLText(DefaultValue)) ' Pointer)
                            Call sqlList.add("HTMLCONTENT", EncodeSQLBoolean(HTMLContent)) ' Pointer)
                            Call sqlList.add("NOTEDITABLE", EncodeSQLBoolean(NotEditable)) ' Pointer)
                            Call sqlList.add("AUTHORABLE", EncodeSQLBoolean(FieldAuthorable)) ' Pointer)
                            Call sqlList.add("EDITARCHIVE", SQLFalse) ' Pointer)
                            Call sqlList.add("EDITBLANK", SQLFalse) ' Pointer)
                            Call sqlList.add("INDEXCOLUMN", EncodeSQLNumber(field.indexColumn)) ' Pointer)
                            Call sqlList.add("INDEXWIDTH", EncodeSQLText(AdminIndexWidth)) ' Pointer)
                            Call sqlList.add("INDEXSORTPRIORITY", EncodeSQLNumber(AdminIndexSort)) ' Pointer)
                            Call sqlList.add("REDIRECTCONTENTID", EncodeSQLNumber(RedirectContentID)) ' Pointer)
                            Call sqlList.add("REDIRECTID", EncodeSQLText(RedirectIDField)) ' Pointer)
                            Call sqlList.add("REDIRECTPATH", EncodeSQLText(RedirectPath)) ' Pointer)
                            Call sqlList.add("UNIQUENAME", EncodeSQLBoolean(UniqueName)) ' Pointer)
                            Call sqlList.add("RSSTITLEFIELD", EncodeSQLBoolean(RSSTitle)) ' Pointer)
                            Call sqlList.add("RSSDESCRIPTIONFIELD", EncodeSQLBoolean(RSSDescription)) ' Pointer)
                            Call sqlList.add("MEMBERSELECTGROUPID", EncodeSQLNumber(MemberSelectGroupID)) ' Pointer)
                            Call sqlList.add("installedByCollectionId", EncodeSQLNumber(InstalledByCollectionID)) ' Pointer)
                            Call sqlList.add("EDITTAB", EncodeSQLText(EditTab)) ' Pointer)
                            Call sqlList.add("SCRAMBLE", EncodeSQLBoolean(Scramble)) ' Pointer)
                            Call sqlList.add("LOOKUPLIST", EncodeSQLText(LookupList)) ' Pointer)
                            Call sqlList.add("MANYTOMANYCONTENTID", EncodeSQLNumber(ManyToManyContentID)) ' Pointer)
                            Call sqlList.add("MANYTOMANYRULECONTENTID", EncodeSQLNumber(ManyToManyRuleContentID)) ' Pointer)
                            Call sqlList.add("MANYTOMANYRULEPRIMARYFIELD", EncodeSQLText(ManyToManyRulePrimaryField)) ' Pointer)
                            Call sqlList.add("MANYTOMANYRULESECONDARYFIELD", EncodeSQLText(ManyToManyRuleSecondaryField)) ' Pointer)
                            Call sqlList.add("ISBASEFIELD", EncodeSQLBoolean(IsBaseField)) ' Pointer)
                            '
                            If RecordID = 0 Then
                                Call sqlList.add("NAME", EncodeSQLText(field.nameLc)) ' Pointer)
                                Call sqlList.add("CONTENTID", EncodeSQLNumber(ContentID)) ' Pointer)
                                Call sqlList.add("CREATEKEY", "0") ' Pointer)
                                Call sqlList.add("DATEADDED", EncodeSQLDate(Now)) ' Pointer)
                                Call sqlList.add("CREATEDBY", EncodeSQLNumber(SystemMemberID)) ' Pointer)
                                '
                ...
                If True Then
                    ...
                    Else
                        ...
                        If (ContentID < 0) Then
                            ...
                            SQL = "Select ID from ccContent where name=" & EncodeSQLText(ContentName)
                            Dim rsContent As DataTable
                        ...
                        For Each dcTableColumns As DataColumn In rsTable.Columns
                            ...
                            Else
                                ...
                                '
                                Call cpCore.app.executeSql("update ccFields Set CreateKey=0 where (Contentid=" & ContentID & ") And (name = " & EncodeSQLText(UcaseTableColumnName) & ")")
                            End If
                        Next
                    End If
                End If
