                    ...
                    If (appConfig.appStatus = Models.Entity.serverConfigModel.appStatusEnum.ready) And (appConfig.appMode = Models.Entity.serverConfigModel.appModeEnum.normal) Then
                        ...
                        If (cp IsNot Nothing) Then
                            If True Then
                                ...
                                If Not addonInstallClass.UpgradeLocalCollectionRepoFromRemoteCollectionRepo(cp.core, ErrorMessage, ignoreRefactorText, ignoreRefactorBoolean, False, nonCriticalErrorList) Then
                                ...
                                '
                                Call addonInstallClass.installCollectionFromRemoteRepo(cp.core, CoreCollectionGuid, ErrorMessage, "", False, nonCriticalErrorList)
                                '
                                ...
                                If RunServerHousekeep Or DebugMode Then
                                    ...
                                    If True Then
                                        ...
                                        Do While (datePtr <= Yesterday)
                                            If Not cp.core.db.csOk(CS) Then
                                                '
                                                ...
                                                'Exit For
                                            Else
                                                workingDate = Date.MinValue.AddDays(cp.core.db.csGetInteger(CS, "DateNumber"))
                                    ...
                                    If True Then ' 4.1.187" Then
                                        ...
                                        If True Then
                                            ...
                                            Else
                                                datePtr = Date.MinValue.AddDays(cp.core.db.csGetInteger(CS, "DateNumber"))
                                            End If
                                            Call cp.core.db.csClose(CS)
...
ErrorTrap:
        ...
        Private Sub HouseKeep_App_Daily(ByVal VisitArchiveAgeDays As Integer, ByVal GuestArchiveAgeDays As Integer, ByVal EmailDropArchiveAgeDays As Integer, ByVal DefaultMemberName As String, ByVal BuildVersion As String)
            ...
            CS = cp.core.db.csOpenSql_rev("default", SQL)
            Do While cp.core.db.csOk(CS)
                Call cp.core.db.deleteContentRecord("Group Rules", cp.core.db.csGetInteger(CS, "ID"))
                Call cp.core.db.csGoNext(CS)
            Loop
            Call cp.core.db.csClose(CS)
            '
            ...
            CS = cp.core.db.csOpenSql_rev("default", SQL)
            Do While cp.core.db.csOk(CS)
                Call cp.core.db.deleteContentRecord("Content Watch", cp.core.db.csGetInteger(CS, "ID"))
                Call cp.core.db.csGoNext(CS)
            Loop
            Call cp.core.db.csClose(CS)
            '
                    ...
                    If FileList.Count > 0 Then
                        ...
                        For Each file As IO.FileInfo In FileList
                            ...
                            Else
                                ...
                                If Not cp.core.db.csOk(CSTest) Then
                                    Call cp.core.cdnFiles.deleteFile(VirtualFileName)
                                End If
                                Call cp.core.db.csClose(CSTest)
                            End If
                        Next
                        If DSType = 1 Then
                            ' access
        ...
        Public Sub HouseKeep_VisitSummary(ByVal StartTimeDate As Date, ByVal EndTimeDate As Date, ByVal HourDuration As Integer, ByVal BuildVersion As String, ByVal OldestVisitSummaryWeCareAbout As Date)
            ...
            Else
                ...
                Do While PeriodDatePtr < EndTimeDate
                    ...
                    '
                    If cp.core.db.csOk(CS) Then
                        Call cp.core.db.csSet(CS, "name", HourDuration & " hr summary for " & Date.FromOADate(DateNumber).ToShortDateString & " " & TimeNumber & ":00")
                        Call cp.core.db.csSet(CS, "DateNumber", DateNumber)
                        Call cp.core.db.csSet(CS, "TimeNumber", TimeNumber)
                        Call cp.core.db.csSet(CS, "Visits", VisitCnt)
                        Call cp.core.db.csSet(CS, "PagesViewed", HitCnt)
                        Call cp.core.db.csSet(CS, "TimeDuration", HourDuration)
                        Call cp.core.db.csSet(CS, "NewVisitorVisits", NewVisitorVisits)
                        Call cp.core.db.csSet(CS, "SinglePageVisits", SinglePageVisits)
                        Call cp.core.db.csSet(CS, "AuthenticatedVisits", AuthenticatedVisits)
                        Call cp.core.db.csSet(CS, "NoCookieVisits", NoCookieVisits)
                        Call cp.core.db.csSet(CS, "AveTimeOnSite", AveTimeOnSite)
                        If True Then
                            Call cp.core.db.csSet(CS, "MobileVisits", MobileVisits)
                            Call cp.core.db.csSet(CS, "BotVisits", BotVisits)
                        End If
                    End If
                    Call cp.core.db.csClose(CS)
        ...
        Public Sub HouseKeep_PageViewSummary(ByVal StartTimeDate As Date, ByVal EndTimeDate As Date, ByVal HourDuration As Integer, ByVal BuildVersion As String, ByVal OldestVisitSummaryWeCareAbout As Date)
            ...
            Else
                ...
                Do While PeriodDatePtr < EndTimeDate
                    ...
                    If Not cp.core.db.csOk(CSPages) Then
                        '
                        ...
                        '
                        If cp.core.db.csOk(CS) Then
                            Call cp.core.db.csSet(CS, "name", HourDuration & " hr summary for " & Date.FromOADate(CDbl(DateNumber)) & " " & TimeNumber & ":00, " & PageTitle)
                            Call cp.core.db.csSet(CS, "DateNumber", DateNumber)
                            Call cp.core.db.csSet(CS, "TimeNumber", TimeNumber)
                            Call cp.core.db.csSet(CS, "TimeDuration", HourDuration)
                            Call cp.core.db.csSet(CS, "PageViews", PageViews)
                            Call cp.core.db.csSet(CS, "PageID", PageID)
                            Call cp.core.db.csSet(CS, "PageTitle", PageTitle)
                            Call cp.core.db.csSet(CS, "AuthenticatedPageViews", AuthenticatedPageViews)
                            Call cp.core.db.csSet(CS, "NoCookiePageViews", NoCookiePageViews)
                            If True Then
                                Call cp.core.db.csSet(CS, "MobilePageViews", MobilePageViews)
                                Call cp.core.db.csSet(CS, "BotPageViews", BotPageViews)
                            End If
                        End If
                        Call cp.core.db.csClose(CS)
                    Else
                        ...
                        '
                        Do While cp.core.db.csOk(CSPages)
                            ...
                            If cp.core.db.csOk(CS) Then
                                ...
                                If PageTitle = "" Then
                                    ...
                                    Call cp.core.db.csSet(CS, "name", HourDuration & " hr summary for " & Date.FromOADate(CDbl(DateNumber)) & " " & TimeNumber & ":00, " & PageName)
                                    Call cp.core.db.csSet(CS, "PageTitle", PageName)
                                Else
                                    Call cp.core.db.csSet(CS, "name", HourDuration & " hr summary for " & Date.FromOADate(CDbl(DateNumber)) & " " & TimeNumber & ":00, " & PageTitle)
                                    Call cp.core.db.csSet(CS, "PageTitle", PageTitle)
                                End If
                            ...
                            End If
                            Call cp.core.db.csClose(CS)
                            Call cp.core.db.csGoNext(CSPages)
                        Loop
                    End If
                    Call cp.core.db.csClose(CSPages)
                    PeriodDatePtr = PeriodDatePtr.AddHours(HourDuration)
                Loop
            End If
            '
